...
public class CyclonePro : ModProjectile
{
    ...
    public override void AI()
    {
				...
				if (projectile.localAI[1] > 10f && Main.rand.Next(3) == 0)
				{
					...
					{
						Vector2 vector65 = Vector2.Normalize(projectile.velocity) * new Vector2((float)projectile.width, (float)projectile.height) / 2f;
						vector65 = vector65.RotatedBy((double)(num817 - (num816 / 2 - 1)) * 3.1415926535897931 / (double)((float)num816), default(Vector2)) + projectile.Center;
				...
				if (projectile.ai[0] == 0f && projectile.localAI[0] == 0f)
				{
					for (int num821 = 0; num821 < 200; num821++)
					{
						...
						if (nPC8.active && !nPC8.dontTakeDamage && !nPC8.friendly && nPC8.lifeMax > 5 && (projectile.ai[0] == 0f || projectile.ai[0] == (float)(num821 + 1)))
						{
					...
					if (num819 >= 0)
					{
						projectile.ai[0] = (float)(num819 + 1);
				...
				if (projectile.ai[0] != 0f)
				{
					...
					if (Main.npc[num823].active && !Main.npc[num823].dontTakeDamage && Main.npc[num823].immune[projectile.owner] == 0)
					{
						...
						float num826 = Math.Abs(projectile.position.X + (float)(projectile.width / 2) - num824) + Math.Abs(projectile.position.Y + (float)(projectile.height / 2) - num825);
						if (num826 < 1000f)
						{
							flag33 = true;

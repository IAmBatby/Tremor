...
public class SuperHealingFlaskPro : ModProjectile
{
            ...
            if (!modPlayer.pyro && !modPlayer.nitro)
            {
                if (projectile.owner == Main.myPlayer)
                {
                    ...
                    for (int num221 = 0; num221 < num220; num221++)
                    {
                        ...
                        value17.Normalize();
                        value17 *= (float)Main.rand.Next(10, 201) * 0.01f;
                        Projectile.NewProjectile(projectile.position.X, projectile.position.Y, value17.X, value17.Y, mod.ProjectileType("HealingCloudPro"), projectile.damage, 1f, projectile.owner, 0f, (float)Main.rand.Next(-45, 1));
                    }
                }

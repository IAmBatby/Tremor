...
namespace Tremor.Items
{
	public class DarknessDust : ModItem
	{
		...
		{
			DisplayName.SetDefault("Darkness Dust");
			Tooltip.SetDefault("");
		}
		...
		public override void UpdateAccessory(Player player, bool hideVisual)
		{
			if (Math.Abs(player.velocity.X) + Math.Abs(player.velocity.Y) > 1f && !player.rocketFrame) // Makes sure the player is actually moving
			{
				for (int k = 0; k < 2; k++)
				{
					int index = Dust.NewDust(new Vector2(player.position.X - player.velocity.X * 2f, player.position.Y - 2f - player.velocity.Y * 2f), player.width, player.height, 27, 0f, 0f, 100, default(Color), 2f);

...
namespace Tremor.NPCs
{
	...
	public class Hellhound : ModNPC
	{
		...
		public override void SetDefaults()
		{
			...
			banner = npc.type;
			bannerItem = mod.ItemType("HellhoundBanner");
		}
		...
		public override void HitEffect(int hitDirection, double damage)
		{
			...
			else
			{
				...
				{
					Dust.NewDust(npc.position, npc.width, npc.height, 6, (float)hitDirection, -2f, 0, default(Color), 1.7f);
					Dust.NewDust(npc.position, npc.width, npc.height, 6, (float)hitDirection, -1f, 0, default(Color), 0.7f);
				}
			}
		}
		...
		public override void AI()
		{
			...
			if (npc.ai[2] == 1f)
			{
				...
				if (npc.ai[1] < 30f)
				{
					Vector2 vector73 = npc.Center + Vector2.UnitX * (float)npc.spriteDirection * -20f;
					...
					dust11.scale = 0.5f + vector74.X * (float)(-(float)npc.spriteDirection);
					dust11.fadeIn = 1f;
				...
				else if (npc.ai[1] == 30f)
				{
					...
					{
						Vector2 vector75 = npc.Center + Vector2.UnitX * (float)npc.spriteDirection * -20f;
						...
						dust12.velocity = vector76 * 4f + Vector2.UnitX * Main.rand.NextFloat() * (float)npc.spriteDirection * -5f;
						dust12.scale = 0.5f + vector76.X * (float)(-(float)npc.spriteDirection);
						dust12.fadeIn = 1f;
				...
				if (npc.ai[1] == 30f && Main.netMode != 1)
				{
					...
					Projectile.NewProjectile(npc.Center.X + (float)(npc.spriteDirection * -20), npc.Center.Y, (float)(npc.spriteDirection * -7), 0f, 467, num744, 0f, Main.myPlayer, (float)npc.target, 0f);
				}
				if (npc.ai[1] >= 60f)
				{

...
animationType = 21;
    ...
    public override void HitEffect(int hitDirection, double damage)
    {
        if(npc.life <= 0)
        {
            ...
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/UndeadWarrior2Gore2"), 1f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/UndeadGore1"), 1f);
            Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/UndeadGore2"), 1f);
        }
}
    ...
    public override float SpawnChance(NPCSpawnInfo spawnInfo)
    {
        ...
        return (Tremor.NormalSpawn(spawnInfo) && Tremor.NoZoneAllowWater(spawnInfo)) && !Main.dayTime && y < Main.worldSurface ? 0.05f : 0f;
    }
}}
...
public class TheDarkEmperor: ModNPC
{
    ...
    public override void HitEffect(int hitDirection, double damage)
    {
        if(npc.life <= 0)
        {
            for(int k = 0; k < 60; k++)
            {
                Dust.NewDust(npc.position, npc.width, npc.height, 191, 2.5f * (float)hitDirection, -2.5f, 0, default(Color), 0.7f);
                Dust.NewDust(npc.position, npc.width, npc.height, 191, 2.5f * (float)hitDirection, -2.5f, 0, default(Color), 0.7f);
                Dust.NewDust(npc.position, npc.width, npc.height, 191, 2.5f * (float)hitDirection, -2.5f, 0, default(Color), 0.7f);
            }
        Main.PlaySound(15, 0);
        ...
        else
        {
            ...
            {
                Dust.NewDust(npc.position, npc.width, npc.height, 191, (float)hitDirection, -2f, 0, default(Color), 0.7f);
                Dust.NewDust(npc.position, npc.width, npc.height, 191, (float)hitDirection, -1f, 0, default(Color), 0.7f);
            }
        }
    }
